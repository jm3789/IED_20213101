...
// turn on the LED if the distance is between dist_min and dist_max
  if(dist_raw < dist_min || dist_raw > dist_max) {
    ...
    if (dist_raw <= (dist_min + dist_max)/2) {
      analogWrite(PIN_LED, 255 * (1 - ((dist_raw - dist_min)) / ((dist_max - dist_min) / 2)));
    } else {
      analogWrite(PIN_LED, 255 * (1 - (dist_max - dist_raw)) / ((dist_max - dist_min) / 2));
    }
  }
...
float USS_measure(int TRIG, int ECHO)
{
  ...
  if(reading < dist_min || reading > dist_max) return dist_raw; // return 0 when out of range.
  return reading;
}
